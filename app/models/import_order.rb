class ImportOrder < ApplicationRecord
  ...
  def update_product_info
    ...
    if product
      product.update(bought_quantity: order_colors.map(&:quantity).try(:sum) + product.bought_quantity)
      ...
      product = Product.create(code: product_code,
        product_type: product_type.to_sym,
        bought_quantity: order_colors.map(&:quantity).try(:sum),
        remain_quantity: order_colors.map(&:quantity).try(:sum))
      order_colors.each do |order_color|
        product.order_colors.create(
          quantity: order_color.quantity,
